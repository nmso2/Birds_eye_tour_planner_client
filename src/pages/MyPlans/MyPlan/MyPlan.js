...
const MyPlan = (props) => {
    const { planId, handleCancelPlan , id} = props;
    ...
    return (
        <div>
            {plans.filter(plan => planId === plan._id).map(purchasePlan => <div className="lg:flex items-baseline justify-end">
                ...
                {purchasePlan.confirmed ? <p className="p-2 pl-5 pr-5 bg-blue-500 text-gray-100 text-lg rounded-lg focus:border-4 border-blue-300">Confirmed</p> : <p className="p-2 pl-5 pr-5 bg-yellow-500 text-gray-100 text-lg rounded-lg focus:border-4 border-yellow-300">Pending</p>
                    }
